...
logLik.unif <- array(NA,dim=c(number_chains, Number_iterations, NumberSamples, NumberLoci, 2))
   ...
   for (locus in 1:NumberLoci){
     if(!is.na(alleles_at_locus[sample,locus])){
       for (allele in 1:alleles_at_locus[sample,locus]){
         logLik.unif[chain, , sample, locus, allele] <- sim.sample[[chain]][,paste("loglik[", sample, ",", locus, ",", allele, "]",sep="")]
       }
     }
   }
 }
