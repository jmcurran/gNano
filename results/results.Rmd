...
```{r}
load("../gNanoPkg/systematic/g-0.Rda")
```
...
```{r}
load("../gNanoPkg/systematic/g-1.Rda")
...
```{r}
load("../gNanoPkg/systematic/g-2.Rda")
...
```{r}
load("../gNanoPkg/systematic/g-3.Rda")
...
```{r}
load("../gNanoPkg/systematic/g-4.Rda")
...
p = g4.df %>% 
  ggplot(aes(x = sigma.sq0/10^5)) +
  geom_density() +
  geom_vline(xintercept = quantile(g4.df$sigma.sq0/10^5, probs=0.025)) +
  geom_vline(xintercept = quantile(g4.df$sigma.sq0/10^5, probs=0.975)) +
  xlab(bquote(sigma[0]^2~(x10^5))) + 
  theme(text = element_text(size = 20), aspect.ratio = 1, plot.background = element_rect(color="black"))
p
```
...
```{r}
load("../gNanoPkg/systematic/ln-0.Rda")
...
```{r}
load("../gNanoPkg/systematic/ln-1.Rda")
...
```{r}
load("../gNanoPkg/systematic/ln-2.Rda")
...
```{r}
load("../gNanoPkg/systematic/ln-3.Rda")
...
```{r}
load("../gNanoPkg/systematic/ln-4.Rda")
...
load("../gNanoPkg/systematic/g-4.Rda")
#load(paste(here(),"/systematic/g-4.Rda",sep=""))
g4.df = data.frame(as.matrix(sim.sample[[1]]))
...
load("../gNanoPkg/systematic/ln-4.Rda")
#load(paste(here(),"/systematic/ln-4.Rda",sep=""))
ln4.df = data.frame(as.matrix(sim.sample[[1]]))
...
#plots densities
p = ggplot(plot.df) 
p = p + geom_density(aes(x = g4, col = "gamma"))
p = p + geom_density(aes(x = ln4, col = "log-normal"))
p = p + xlab(expression(log[10]~(likelihood)))
p = p + ylab("density")
p = p + theme(text = element_text(size = 20), aspect.ratio = 1, plot.background = element_rect(color="black"))
p = p + guides(fill=guide_legend(title = "distribution"))
p
