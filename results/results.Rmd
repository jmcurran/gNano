...
```{r}
obsExpectPlot(g1.df, "g", predRes = TRUE, TRUE)
```
...
```{r}
g4.predRes = obsExpectPlot(g4.df, "g", TRUE, TRUE)
```
...
## Does the removal of sum constraints matter  when the effect means are zero in the prior?
...
```{r}
#effectsPlot(ln6.df, "locus")
```
Hmm - similar

```{r}
#effectsPlot(ln6.df, "dye")
```
Just weird!

```{r}
#obsExpectPlot(ln6.df, "ln")
```
No real change here in terms of predicted values

```{r}
#obsExpectPlot(ln6.df, "ln", TRUE)
```
What about change to the log-likelihood?
