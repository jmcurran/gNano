...
ys = g4.df[grep("pred", colnames(g4.df))]
shapes = g4.df[grep("shape", colnames(g4.df))]
rates = g4.df[grep("rate", colnames(g4.df))]
#calculate log(liks)
g4.LL = (shapes-1)*log(ys)-rates+shapes*log(rates)-log(gamma(shapes))
#sum across peaks (just the finite values)
g4.LLsums = as.vector(rowSums(g4.LL))[is.finite(as.vector(rowSums(g4.LL)))]
...
lnYs = ln4.df[grep("pred", colnames(ln4.df))]
mus = ln4.df[grep("mu.", colnames(ln4.df))]
taus = ln4.df[grep("tau.", colnames(ln4.df))]
taus = taus[-4070] #gets rid of the last entry "tau0" which still seems to come through using grep on "tau."
sigmas_sq = 1/(taus*taus)
#calculate log(liks)
ln4.LL = -0.5*((lnYs-mus)/sigmas_sq+0.5 * log(2*pi*sigmas_sq))
#sum across peaks (just the finite values)
ln4.LLsums = as.vector(rowSums(ln4.LL))[is.finite(as.vector(rowSums(ln4.LL)))]

#plots densities
p = ggplot(data.frame(x = g4.LLsums), aes(x = x))
p = p + ggplot(data.frame(x = ln4.LLsums), aes(x = x))
p = p + geom_density()
p
