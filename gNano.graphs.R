createGraphs <- function(sim.sample, saveDir, bugsData) {
  ...
  lambda_capture <- NULL
  try(lambda_capture <- sim.sample[[1]][, "lambda"])
  #amp efficiency mean capture
  A_mu_capture <- NULL
  #counts the occurances of mus
  #if(sum(colnames(sim.sample[[1]])=="mu.amp[1]")>0){
  try(A_mu_capture <- sim.sample[[1]][, "mu.amp[1]"])
  #}
  #if(sum(colnames(sim.sample[[1]])=="mu.amp[2]")>0){
  try(for (locus in 2:NumberLoci) {
    A_mu_capture <-
      cbind(A_mu_capture, sim.sample[[1]][, paste("mu.amp[", locus, "]", sep =
                                                    "")])
  })
  #}
  #dye effect mean capture
  D_mu_capture <- NULL
  try(D_mu_capture <- sim.sample[[1]][, "mu.dye[1]"])
  try(for (dye in 2:NumberDyes) {
    D_mu_capture <-
      cbind(D_mu_capture, sim.sample[[1]][, paste("mu.dye[", dye, "]", sep =
                                                    "")])
  })
  #amp efficiency sigma capture
  A_sigma_capture <- NULL
  try(A_sigma_capture <- sim.sample[[1]][, "sigma.sq.amp[1]"])
  try(for (locus in 2:NumberLoci) {
    A_sigma_capture <-
      cbind(A_sigma_capture, sim.sample[[1]][, paste("sigma.sq.amp[", locus, "]", sep =
                                                       "")])
  })
  #dye effect sigma capture
  D_sigma_capture <- NULL
  try(D_sigma_capture <- sim.sample[[1]][, "sigma.sq.dye[1]"])
  try(for (dye in 2:NumberDyes) {
    D_sigma_capture <-
      cbind(D_sigma_capture, sim.sample[[1]][, paste("sigma.sq.dye[", dye, "]", sep =
                                                       "")])
  })
  ...
  if (!is.null(lambda_capture)) {
    ...
    dev.copy(
      jpeg,
      file = paste(saveDir, "lambda_MCMC.jpg", sep = ""),
      height = 2000,
      width = 2000,
      res = 300
    )
    ...
    binwidth <-
      hist(lambda_capture[burnin:endcapture])$mids[2] - hist(lambda_capture[burnin:endcapture])$mids[1]
    hist(
      lambda_capture[burnin:endcapture],
      xlab = "lambda",
      main = paste("lambda: N(", round(mean(
        lambda_capture[burnin:endcapture]
      ), 3), ",", round(sd(
        lambda_capture[burnin:endcapture]
      ), 3), ")", sep = "")
    ...
    plot_x <-
      seq(min(lambda_capture[burnin:endcapture]), max(lambda_capture[burnin:endcapture]), length =
            1000)
    lines(plot_x,
          binwidth * (endcapture - burnin) * dnorm(plot_x, mean(lambda_capture[burnin:endcapture]), sd(lambda_capture[burnin:endcapture])),
          col = "red")
    dev.copy(
      jpeg,
      file = paste(saveDir, "Lambda_histogram.jpg", sep = ""),
      height = 2000,
      width = 2000,
      res = 300
    )
  ...
  if (!is.null(A_mu_capture)) {
    ...
    plot(
      A_mu_capture[1:endcapture, 1],
      type = "l",
      ylim = c(0, ylim_max),
      xlab = "iteration",
      ylab = "Locus amplification efficiency",
      xlim = c(1, (1.25 * endcapture))
    )
    for (locus in 2:NumberLoci) {
      lines(A_mu_capture[1:endcapture, locus],
            ylim = c(0, ylim_max),
            xlim = c(1, (1.25 * endcapture)))
    }
    #set up labels
    ...
    legend("right", y_label[order(ordered(-x_label))], cex = 0.7, y.intersp =
             0.5)
    
    dev.copy(
      jpeg,
      file = paste(saveDir, "Amp_efficiencies_MCMC.jpg", sep = ""),
      height = 3000,
      width = 3000,
      res = 300
    )
  ...
  if (!is.null(A_sigma_capture)) {
    plot(
      A_sigma_capture[1:endcapture, 1],
      type = "l",
      ylim = c(0.00000001, 4),
      xlab = "iteration",
      ylab = "Locus amplification efficiency variance",
      xlim = c(1, (1.25 * endcapture)),
      log = "y"
    )
    for (locus in 2:NumberLoci) {
      lines(A_sigma_capture[1:endcapture, locus],
            ylim = c(0.00000001, 4),
            xlim = c(1, (1.25 * endcapture)))
    }
    #set up labels
    ...
    dev.copy(
      jpeg,
      file = paste(saveDir, "Amp_efficiencies_sigma_MCMC.jpg", sep = ""),
      height = 3000,
      width = 4000,
      res = 300
    )
  ...
  if (!is.null(D_mu_capture)) {
    plot(
      D_mu_capture[1:endcapture, 1],
      type = "l",
      ylim = c(0, 1.5),
      xlab = "iteration",
      ylab = "Dye amplification efficiency",
      xlim = c(1, (1.25 * endcapture))
    )
    for (locus in 2:NumberDyes) {
      lines(D_mu_capture[1:endcapture, locus],
            ylim = c(0, 1.5),
            xlim = c(1, (1.25 * endcapture)))
    }
    #set up labels
    ...
    dev.copy(
      jpeg,
      file = paste(saveDir, "Dye_effects_MCMC.jpg", sep = ""),
      height = 2000,
      width = 3000,
      res = 300
    )
  ...
  if (!is.null(D_sigma_capture)) {
    plot(
      D_sigma_capture[1:endcapture, 1],
      type = "l",
      ylim = c(0, 0.05),
      xlab = "iteration",
      ylab = "Dye amplification efficiency variance",
      xlim = c(1, (1.25 * endcapture))
    )
    for (locus in 2:NumberDyes) {
      lines(D_sigma_capture[1:endcapture, locus],
            ylim = c(0, 0.05),
            xlim = c(1, (1.25 * endcapture)))
    }
    #set up labels
    ...
    dev.copy(
      jpeg,
      file = paste(saveDir, "Dye_effects_sigma_MCMC.jpg", sep = ""),
      height = 2000,
      width = 3000,
      res = 300
    )
  ...
  if (!is.null(A_mu_capture) && !is.null(A_sigma_capture)) {
    ...
    for (locus in 1:NumberLoci) {
      ...
      ylim_max <-
        3 * dlnorm(A_mu_mean, log(A_mu_mean), sqrt(A_sigma_mean))
      #opens blank plot
      plot(
        0,
        0,
        xlim = c(0, (2 * A_mu_mean)),
        ylim = c(0, ylim_max),
        xlab = "Amplification effiency",
        ylab = "",
        main = paste("locus ", LocusNames[locus], " amplification efficiency", sep =
                       "")
      )
      ...
      for (randcurve in 1:simulated_curves) {
        ...
        lines(
          plot_x,
          dlnorm(plot_x, log(random_A_mu), sqrt(random_A_sigma)),
          col = color,
          xlim = c(0, (2 * A_mu_mean)),
          ylim = c(0, ylim_max)
        )
      ...
      lines(
        plot_x,
        dlnorm(plot_x, log(A_mu_mean), sqrt(A_sigma_mean)),
        col = "red",
        xlim = c(0, (2 * A_mu_mean)),
        ylim = c(0, ylim_max)
      )
      ...
      dev.copy(
        jpeg,
        file = paste(saveDir, LocusNames[locus], " Amp efficiency.jpg", sep = ""),
        height = 2000,
        width = 2000,
        res = 300
      )
  ...
  if (!is.null(D_mu_capture) && !is.null(D_sigma_capture)) {
    ...
    plot(
      ...
      xlim = c(0, 2),
      ylim = c(0, ylim_max),
      xlab = "Dye amplification effiency",
      ylab = "",
      main = ""
    )
    for (dye in 1:NumberDyes) {
      ...
      for (randcurve in 1:simulated_curves) {
        ...
        lines(
          plot_x,
          dlnorm(plot_x, log(random_D_mu), sqrt(random_D_sigma)),
          col = color,
          xlim = c(0, 2),
          ylim = c(0, ylim_max)
        )
      ...
      lines(
        plot_x,
        dlnorm(plot_x, log(D_mu_mean), sqrt(D_sigma_mean)),
        col = DyeColours[dye],
        xlim = c(0, 2),
        ylim = c(0, ylim_max)
      )
    ...
    dev.copy(
      jpeg,
      file = paste(saveDir, "Dye efficiency.jpg", sep = ""),
      height = 2000,
      width = 2000,
      res = 300
    )
  ...
  hist(
    profileData,
    main = "observed peak heights",
    xlab = "peak height (rfu)",
    xlim = c(0, 30000)
  )
  ...
  dev.copy(
    jpeg,
    file = paste(saveDir, "Observed_peak_heights.jpg", sep = ""),
    height = 1500,
    width = 3000,
    res = 300
  )
  ...
  plot(
    ...
    xlab = "Template",
    ylab = "O/E",
    main = paste("Peak Height varibility", sep = "")
  )
  for (sample in 1:NumberSamples) {
    points(
      seq(T[sample], length = length(E_over_O[sample, , ])),
      E_over_O[sample, , ],
      xlim = c(0, xlim_range),
      ylim = c(0, 4)
    )
      ...
      if (!is.na(alleles_at_locus[sample, locus])) {
        for (allele in 1:alleles_at_locus[sample, locus]) {
          if (!is.na(E_over_O[sample, locus, allele])) {
            if (E_over_O[sample, locus, allele] <= exp(sqrt(Z_value * mean_lambda /
                                                            T[sample])) &&
                E_over_O[sample, locus, allele] >= exp(-sqrt(Z_value * mean_lambda / T[sample]))) {
  ...
  percent_within <-
    round(100 * count_within_bounds / (count_within_bounds + count_outside_bounds),
          2)
  legend(
    "topright",
    c(
  ...
  dev.copy(
    jpeg,
    file = paste(saveDir, "Peak_Height_Variability.jpg", sep = ""),
    height = 2000,
    width = 3000,
    res = 300
  )
  ...
  if (!is.null(D_mu_capture) && !is.null(D_sigma_capture)) {
    ...
    for (combo in 1:combinations) {
      dye1_dye2_observed <-
        Hb[(profileDyes[, , 1] == dye1[combo] &
              profileDyes[, , 2] == dye2[combo])]
      ...
      for (var in 1:1000) {
        ...
        peak1_variability[var] <-
          rlnorm(1, log(peak), sqrt(mean_lambda / peak)) / rlnorm(1, log(peak), sqrt(mean_lambda /
                                                                                       peak))
      }
      dye1_dye2_simulated <-
        peak1_variability * dye1_simulated / dye2_simulated
      if (combo == 1) {
        boxplot(
          dye1_dye2_observed,
          at = 1,
          xlim = c(1, combinations * 2),
          ylim = c(0, 7),
          main = "observed (O) and simulated (S) dye balance",
          xlab = "",
          ylab = "Ratio of Heights",
          col = "red"
        )
      }
      else {
    ...
    dev.copy(
      jpeg,
      file = paste(saveDir, "Dye_amplification_Efficiency.jpg", sep = ""),
      height = 2000,
      width = 6000,
      res = 300
    )
  ...
  if (!is.null(A_mu_capture) && !is.null(A_sigma_capture)) {
    Av_peakheight_per_profile <-
      apply(profileData, c(1), mean, na.rm = TRUE)
    scaled_O  <- array(data = NA,
                       dim = c(NumberSamples, NumberLoci, 2))
    #add across peaks of heterozygote
    scaled_O  <- apply(profileData, c(1, 2), sum, na.rm = TRUE)
    for (profile in 1:NumberSamples) {
      scaled_O[profile, ] <-
        scaled_O[profile, ] / Av_peakheight_per_profile[profile]
    }
    #plot as boxplot
    for (locus in 1:NumberLoci) {
      if (locus == 1) {
        boxplot(
          scaled_O[, locus],
          at = (2 * (locus - 1)),
          xlim = c(1, NumberLoci * 2),
          ylim = c(0, 6),
          main = "observed (O) and simulated (S) lous amp balance",
          xlab = "",
          ylab = "ratio peak heights / average",
          col = "red"
        )
      }
      else {
    ...
    for (var in 1:1000) {
      ...
      for (locus in 1:NumberLoci) {
        ...
        peak_variability <-
          rlnorm(1,
                 log(random_template),
                 sqrt(mean_lambda / random_template))
        #random locus amp efficiency value
        random_locus_amp <-
          rlnorm(1, log(A_mu_means), sqrt(A_sigma_means))
        random_profile[var, locus] <- peak_variability * random_locus_amp
      }
      #scale
      random_profile[var, ] <-
        random_profile[var, ] / mean(random_profile[var, ])
    }
    #plot simulated data
    for (locus in 1:NumberLoci) {
      boxplot(
        random_profile[, locus],
        at = (2 * (locus - 1) + 1),
        xlim = c(1, NumberLoci * 2),
        ylim = c(0, 6),
        col = "blue",
    ...
    axis(1,
         at = seq(1, (2 * NumberLoci - 1), length = NumberLoci),
         labels = LocusNames)
    dev.copy(
      jpeg,
      file = paste(saveDir, "Locus_amplification_Efficiency.jpg", sep = ""),
      height = 2000,
      width = 6000,
      res = 300
    )
  ...
  #values for graph
  number_points <-
    1 + 1 + NumberDyes + NumberDyes + NumberLoci + NumberLoci
  ...
  for (dye in 1:NumberDyes) {
    try(GR <- gelman.diag(sim.sample[, paste("mu.dye[", dye, "]", sep = "")]))
    try(GR_D_mu[dye] <- GR[[1]][1])
    try(graph_values[graph_counter] <- GR_D_mu[dye])
    graph_labels[graph_counter] <-
      paste("Dye amp mean (", DyeNames[dye], ")", sep = "")
    graph_counter <- graph_counter + 1
  }
  #D sigma
  ...
  for (dye in 1:NumberDyes) {
    try(GR <-
          gelman.diag(sim.sample[, paste("sigma.sq.dye[", dye, "]", sep = "")]))
    try(GR_D_sigma[dye] <- GR[[1]][1])
    try(graph_values[graph_counter] <- GR_D_sigma[dye])
    graph_labels[graph_counter] <-
      paste("Dye amp var (", DyeNames[dye], ")", sep = "")
    graph_counter <- graph_counter + 1
  }
  #A mu
  GR_A_mu <- array(data = NA, dim = c(NumberLoci))
  for (locus in 1:NumberLoci) {
    try(GR <- gelman.diag(sim.sample[, paste("mu.amp[", dye, "]", sep = "")]))
    try(GR_A_mu[locus] <- GR[[1]][1])
    try(graph_values[graph_counter] <- GR_A_mu[locus])
    graph_labels[graph_counter] <-
      paste("Locus amp mean (", LocusNames[locus], ")", sep = "")
    graph_counter <- graph_counter + 1
  }
  #A sigma
  ...
  for (locus in 1:NumberLoci) {
    try(GR <-
          gelman.diag(sim.sample[, paste("sigma.sq.amp[", dye, "]", sep = "")]))
    try(GR_A_sigma[locus] <- GR[[1]][1])
    try(graph_values[graph_counter] <- GR_A_sigma[locus])
    graph_labels[graph_counter] <-
      paste("Locus amp var (", LocusNames[locus], ")", sep = "")
    graph_counter <- graph_counter + 1
  }
  graph_labels <-
    paste(graph_labels, " : ", round(graph_values, 2), sep = "")
  #now draw the graph
  plot (
    graph_values,
    main = "Gelman Rubin convergence diagnostic",
    xlab = "Parameter",
    ylab = "GR value",
    xaxt = 'n',
    ylim = c(0, 12)
  ...
  text(
    seq(1, (graph_counter - 1), length = (graph_counter - 1)),
    (graph_values + 2.5),
    labels = graph_labels,
    pos = 3,
    cex = 0.7,
  ...
  dev.copy(
    jpeg,
    file = paste(saveDir, "parameter_GR.jpg", sep = ""),
    height = 2000,
    width = 6000,
    res = 300
  )
